...
namespace BizSim.Google.Play.AssetDelivery
{
    ...
    /// <c>com.bizsim.google.play.assetdelivery.AssetDeliveryBridge$IFetchCallback</c>.
    /// Method names and signatures MUST match the Java interface verbatim (case-sensitive).
    /// </summary>
    /// <remarks>
    /// The Java bridge passes five parallel primitive arrays to avoid per-field JNI accessor calls.
    /// <c>onFetchSettled</c> zips them into an <see cref="AssetPackStates"/> dictionary on the
    /// main thread.
    /// </remarks>
    internal sealed class FetchCallbackProxy : AndroidJavaProxy
    {
        private readonly TaskCompletionSource<AssetPackStates> _tcs;

        public FetchCallbackProxy(TaskCompletionSource<AssetPackStates> tcs)
            : base("com.bizsim.google.play.assetdelivery.AssetDeliveryBridge$IFetchCallback")
        {
            _tcs = tcs ?? throw new ArgumentNullException(nameof(tcs));
        }

        // Signature must match Java IFetchCallback.onFetchSettled exactly.
        public void onFetchSettled(
            string[] packNames,
            int[]    statuses,
            int[]    errorCodes,
